...
class DeepSVDDTrainer(BaseTrainer):
    ...
    def __init__(self, objective, R, c, nu: float, update_thres: int, dp_sigma: float, clip_coeff: float, optimizer_name: str = 'adam', lr: float = 0.001, n_epochs: int = 150,
        ...
        if self.n_epochs >= 0:
            ...
            for epoch in range(self.n_epochs):
                ...
                for data in train_loader:
                    ...
                    ## clipping
                    torch.nn.utils.clip_grad_norm_(net.parameters(), self.clip_coeff)
                    for name, param in net.named_parameters():
                        clipped_grads[name] += param.grad

                    ## add noise
                    for name, param in net.named_parameters():
                        param.grad = (clipped_grads[name] + torch.FloatTensor(clipped_grads[name].size()).normal_(0, self.dp_sigma * self.clip_coeff).cuda())

                    ###################################################
                    #######
                    optimizer.step()
                    update_num += 1
                    if update_num >= self.update_thres:
                        flag = 1
                        break
    ...
    def test(self, dataset: BaseADDataset, net: BaseNet):
        ...
        # Testing
        ##################################################################################################################
        logger.info('Starting trainset testing...')
        start_time = time.time()
        idx_label_score = []
        net.eval()
        with torch.no_grad():
        ...
        #logger.info('Trainset testing time: %.3f' % self.test_time)
