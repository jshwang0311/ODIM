...
from base.odds_random_dataset import ODDSRandomDataset
from .preprocessing import create_semisupervised_setting

import torch


class ODDSADRandomDataset(BaseADDataset):
    ...
    def __init__(self, root: str, dataset_name: str, n_known_outlier_classes: int = 0, ratio_known_normal: float = 0.0,
        ...
        # Get train set
        train_set = ODDSRandomDataset(root=self.root, dataset_name=dataset_name, train=True, feature_range = feature_range, random_state=random_state,
                                download=True)
        ...
        # Get test set
        self.test_set = ODDSRandomDataset(root=self.root, dataset_name=dataset_name, train=False, feature_range = feature_range, random_state=random_state)
