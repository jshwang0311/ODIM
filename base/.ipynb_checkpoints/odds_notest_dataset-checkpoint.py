...
class ODDS_noTest_Dataset(Dataset):
    """
    ...
    def __init__(self, root: str, dataset_name: str, train=True, random_state=None, download=False):
        ...
        self.file_name = self.dataset_name.replace('_notest','') + '.mat'
        ...
        X_train_out, X_test_out, y_train_out, y_test_out,idx_train_out,idx_test_out = train_test_split(X[idx_out], y[idx_out],idx_total[idx_out],
                                                                            test_size=0.4,
                                                                            random_state=random_state)
        X_train = np.concatenate((X_train_norm, X_train_out, X_test_norm, X_test_out))
        #X_test = np.concatenate((X_test_norm, X_test_out))
        y_train = np.concatenate((y_train_norm, y_train_out, y_test_norm, y_test_out))
        #y_test = np.concatenate((y_test_norm, y_test_out))
        idx_train = np.concatenate((idx_train_norm, idx_train_out, idx_test_norm, idx_test_out))
