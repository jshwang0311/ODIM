...
if __name__ == "__main__":
    ...
    for normal_class_idx in range(len(normal_class_list)):
        ...
        for seed_idx in range(len(data_seed_list)):
            ...
            save_dir = os.path.join(f'Results/{dataset_name}/ODIM_fully_fit_{filter_net_name}',f'log{seed}')
            os.makedirs(save_dir, exist_ok=True)
            save_score_dir = os.path.join(f'Results/{dataset_name}/ODIM_fully_fit_{filter_net_name}',f'score{seed}')
            ...
            for model_iter in range(n_ens):
                ...
                ## step 1
                train_idxs_losses, test_idxs_losses = odim_fully_fit(filter_net_name, train_loader, test_loader, check_iter, patience, model_seed,seed, logger, train_option)
        ...
        train_df.to_csv(os.path.join(save_metric_dir,f'ODIM_fully_fit_{filter_net_name}_train_result.csv'))
        ...
        test_df.to_csv(os.path.join(save_metric_dir,f'ODIM_fully_fit_{filter_net_name}_test_result.csv'))
